...
pygame.display.set_caption("Calcutron 3000 by: Macías Luciano y Payajo Shein") # Nombre de la calculadora
...
class Calculadora:
	...
	def dibujar(self,ventana):
		...
		creditos = funte.render("Calcutron 3000 by: Macías Luciano y Payajo Shein",0,gras) # Marca de agua
	...
	def infomenu(self,ventana):
		...
		textto = funte.render("1. Todas las operaciones compuestas son facilmente realizables.",0,blanco)
		separacion = 12+th
		...
		tw,th = funte.size("I")
		separacion += th
		textto = funte.render("2. Tener cuidado con el limite de caracteres disponibles.",0,blanco)
		ventana.blit(textto,(20,separacion))
		pygame.display.flip() # Actualizamos la ventana

		tw,th = funte.size("I")
		separacion += th + 5
		textto = funte.render("3. '%' se utiliza para sacar el resto entre dos numeros y no para porentajes.",0,blanco)
		ventana.blit(textto,(20,separacion))
		pygame.display.flip() # Actualizamos la ventana
		...
		tw,th = funte.size("I")
		separacion += th + 5
		textto = funte.render("5. Respetar el uso de parentesis cuando se dibujan en pantalla automaticamente.",0,blanco)
		ventana.blit(textto,(20,separacion))
		pygame.display.flip() # Actualizamos la ventana

		tw,th = funte.size("I")
		separacion += th
		textto = funte.render("6. Las operaciones como 'sin', 'cos' y 'tan', devuelven los valores en rad.",0,blanco)
		ventana.blit(textto,(20,separacion))
		pygame.display.flip() # Actualizamos la ventana

		tw,th = funte.size("I")
		separacion += th + 5
		textto = funte.render("7. Luego de presionar la igualacion '=' es necesario limpiar con el boton 'AC'.",0,blanco)
		ventana.blit(textto,(20,separacion))
		pygame.display.flip() # Actualizamos la ventana

		tw,th = funte.size("I")
		separacion += th
		textto = funte.render("9. El numero maximo de caracteres es de 47.",0,blanco)
		ventana.blit(textto,(20,separacion))
		pygame.display.flip() # Actualizamos la ventana

		tw,th = funte.size("I")
		separacion += th + 5
		textto = funte.render("10. Al presionar la potenciacion, todo lo que haya en el visor será elevado,",0,blanco)
		ventana.blit(textto,(20,separacion))
		pygame.display.flip() # Actualizamos la ventana

		tw,th = funte.size("I")
		separacion += th
		textto = funte.render("utilizar con sabiduria.",0,blanco)
		ventana.blit(textto,(20,separacion))
		pygame.display.flip() # Actualizamos la ventana
		...
		separacion += th+20
		funte = pygame.font.SysFont("Times New Roman", 24)
		textto = funte.render("Calcutron 3000 by: Macías Luciano y Payajo Shein",0,naranja)
		...
		if valor != "=" and valor != "AC" and valor != "Info" and self.limite == False:
			try:
				if valor == "x":
					...
					self.expresion += "sin("# Expresion guardará el código
					self.visor += "sin("# visor mostrará en pantalla
				elif valor == "cos":
					self.expresion += "cos("# Expresion guardará el código
					self.visor += "cos("# visor mostrará en pantalla
				elif valor == "tan":
					self.expresion += "tan("# Expresion guardará el código
